    ...
    try:
        ...
        if need_rag and not is_greeting:
            full_context, rag_results = await asyncio.gather(
                ...
                rag_service.retrieve_knowledge(rewritten_query),
            )
        else:
        ...
        # Step 6: turn + stage 결정
        new_turn_count = turn_count + 1
        new_stage = "diagnosis" if new_turn_count >= 4 else "questioning"
...
async def _persist_message_and_update(
    ...
    try:
        ...
        await redis_service.set_session_state(
            session_id,
            {"turn_count": str(turn_count), "stage": stage, "lang": lang, "last_updated": now_iso},
        )
