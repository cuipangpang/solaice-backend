...
FAST_SYSTEM_TEMPLATE = """{pinned_context}{prefix}
...
THINKING_SYSTEM_TEMPLATE = """{pinned_context}{prefix}
...
【사고 공간 (XML)】
답변을 작성하기 전 반드시 <thinking_process> 태그 안에서 다음을 검토하세요:
1. RAG 문서들이 서로 모순되는가?
2. 증상 원인이 논리적으로 일치하는가?
3. 심각한 합병증을 놓치고 있지 않은가?
검토 완료 후 <thinking_process> 태그 바깥에 최종 답변을 작성하세요.
...
def _format_structured_facts(sf: dict) -> str:
    """하위 호환 래퍼 — _format_pinned_context 로 이전."""
    return _format_pinned_context({}, sf)


def _format_pinned_context(pet_profile: dict, sf: dict) -> str:
    """
    Pinned Context 블록.
    pet_profiles(이름·종·나이) + structured_facts(알레르기·약물·확진)를
    system prompt 맨 앞에 고정.

    경고문: "이하 정보는 절대적 사실입니다. 어떤 답변도 이를 위반하면 안 됩니다."
    """
    allergies_list = (sf or {}).get("allergies") or []
    conditions_list = (sf or {}).get("diagnosed_conditions") or []
    medications_list = (sf or {}).get("medications") or []
    ...
    profile_str = " | ".join(filter(None, [
        ...
        f"종류: {species}" if species else "",
        f"나이: {age}살" if age else "",
    ]))

    lines = [
        "╔═══════════════════════════════════════════════════════╗",
        "  이하 정보는 절대적 사실입니다. 어떤 답변도 이를 위반하면 안 됩니다.",
    ]
    if profile_str:
        lines.append(f"  반려동물: {profile_str}")
    lines += [
        f"  알레르기: {', '.join(allergies_list) if allergies_list else '없음'}",
        f"  복용 약물: {', '.join(medications_list) if medications_list else '없음'}",
        f"  확진 질병: {', '.join(conditions_list) if conditions_list else '없음'}",
        "╚═══════════════════════════════════════════════════════╝\n",
    ]
    return "\n".join(lines)
...
def build_chat_prompt(
    ...
    # ── system_prompt 생성 ────────────────────────────────────────────
    if mode == "fast":
        pinned = _format_pinned_context(pet_profile, structured_facts or {})
